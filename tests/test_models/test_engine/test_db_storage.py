#!/usr/bin/python3
""" This module defines for testing database storage"""
...
class TestDBStorage(unittest.TestCase):
    ...
    def test_new(self):
        """ Checks new object is correctly added to database """
        new = User(
            email='mhr@gmail.com',
            password='password',
            first_name='Mahari',
        ...
        self.assertIn('mhr@gmail.com', result)
        self.assertIn('password', result)
        self.assertIn('Mahari', result)
        self.assertIn('Tsegay', result)
        cursor.close()
        dbc.close()

    def test_reload(self):
        """ Checks the reloading of the database"""
        dbc = MySQLdb.connect(
            host=os.getenv('HBNB_MYSQL_HOST'),
            port=3306,
            user=os.getenv('HBNB_MYSQL_USER'),
    ...
    def test_delete(self):
        """ Checks object is correctly deleted from database """
        new = User(
            email='mhr@gmail.com',
            password='password',
            first_name='Mahari',
    ...
    def test_save(self):
        """ Checks object is successfully saved to database """
        new = User(
            email='mhr@gmail.com',
            password='password',
            first_name='Mahari',
    ...
    def test_storage_var_created(self):
        """ Checks DBStorage object storage created """
        from models.engine.db_storage import DBStorage
        self.assertEqual(type(storage), DBStorage)

    def test_new_and_save(self):
        '''Checks the new and save methods'''
        db = MySQLdb.connect(user=os.getenv('HBNB_MYSQL_USER'),
                             host=os.getenv('HBNB_MYSQL_HOST'),
                             passwd=os.getenv('HBNB_MYSQL_PWD'),
                             port=3306,
                             db=os.getenv('HBNB_MYSQL_DB'))
        new_user = User(**{'first_name': 'Abiel',
                           'last_name': 'Eyasu',
                           'email': 'abiel@eyasu.com',
                           'password': 12345})
        cur = db.cursor()
